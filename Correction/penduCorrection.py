...
from sources import donneesc
from Fonction import fonctionsc

# On récupère les scores de la partie
scores = fonctionsc.recup_scores()

# On récupère un nom d'utilisateur
utilisateur = fonctionsc.recup_nom_utilisateur()
...
while continuer_partie != 'n':
    ...
    mot_a_trouver = fonctionsc.choisir_mot()
    lettres_trouvees = []
    mot_trouve = fonctionsc.recup_mot_masque(mot_a_trouver, lettres_trouvees)
    nb_chances = donneesc.nb_coups
    while mot_a_trouver!=mot_trouve and nb_chances>0:
        print("Mot à trouver {0} (encore {1} chances)".format(mot_trouve, nb_chances))
        lettre = fonctionsc.recup_lettre()
...
fonctionsc.enregistrer_scores(scores)
